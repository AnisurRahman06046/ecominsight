"""Hybrid orchestrator using ML-based intent classification."""

import time
from typing import Dict, Any, Optional
import logging

from app.services.hybrid_intent_classifier import hybrid_intent_classifier
...
class HybridOrchestrator:
    """Orchestrator using hybrid intent classification (rules + ML)."""

    def __init__(self):
        self.intent_classifier = hybrid_intent_classifier
    ...
    async def process_query(
        ...
        """
        Process query using hybrid classification.
        ...
        if use_cache and settings.enable_cache:
            cached_result = await self.cache.get(f"hybrid:{shop_id}:{question}")
            if cached_result:
        ...
        # Classify intent (hybrid approach)
        intent, params, method = await self.intent_classifier.classify(
            question, use_ml=use_ml
        )

        logger.info(
            f"Intent: {intent.value}, Method: {method}, Params: {params}"
        )
        ...
        if intent == Intent.GREETING:
            ...
            result = await self._handle_unknown(question)
        else:
            result = await self._handle_intent(
                intent, int(shop_id), params, question
            )
    ...
    async def _handle_intent(
        ...
        """Handle a recognized intent by calling appropriate tool."""
        try:
            tool_function = self.intent_to_function.get(intent)
            if not tool_function:
                logger.warning(f"No function mapped for intent: {intent.value}")
                return await self._handle_unknown(question)

            logger.info(f"Calling tool: {tool_function.__name__}")
            tool_result = await tool_function(shop_id, **params)

            # Format answer
            answer = self._format_answer(question, tool_result, intent.value)
    ...
    def _format_answer(
        self, question: str, tool_result: Dict[str, Any], intent: str
    ) -> str:
        """Format answer using simple templates (fast, accurate)."""
        # Simple template-based formatting (no LLM needed)
        ...
        if "top_customers" in tool_result:
            ...
            if customers:
                customer_list = "\n".join(
                    [
                        f"{i+1}. {c.get('customer_name', 'Unknown')} - ${c.get('total_spent', 0):,.2f}"
    ...
    async def _handle_greeting(self) -> Dict[str, Any]:
        """Handle greeting."""
        return {
            "answer": "Hello! I'm your e-commerce analytics assistant. Ask me about your products, orders, customers, or revenue.",
            "data": None,
            "metadata": {},
        }

    async def _handle_unknown(self, question: str) -> Dict[str, Any]:
